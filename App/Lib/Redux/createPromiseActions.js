...
const addSupportTypes = (types) => types.reduce(
  (acc, type) => acc.concat(
    type,
    type + '_REQUEST',
    type + '_SUCCESS',
...
const createActionCreator = (name, extraPropNames, options) => {
  ...
  if (noKeys) return () => createPromise({ type })
  ...
  if (is(Array, extraPropNames)) {
    return (...values) => {
      ...
      return createPromise({ type, payload: { ...extraProps }})
    }
  }
  ...
  if (is(Object, extraPropNames)) {
    ...
    return (valueObject) => {
      ...
      return createPromise({ type, payload: { ...defaultProps, ...providedProps }})
    }
  }
